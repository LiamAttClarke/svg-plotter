...
const rectTransformer: SVGNodeTransformer = (input, svgMeta, options) => {
    ...
    const ry = mathUtils.clamp(
        parseFloat(input.attributes.ry), 0, height * 0.5,
    );
    const ring: Vector2[] = [];
    if (rx || ry) {
        const topLeftCorner = mathUtils.drawCurve(
            (t) => mathUtils.pointOnEllipse(
                new Vector2(x + rx, y + ry), rx, ry, t,
            ),
        ...
        const topRightCorner = mathUtils.drawCurve(
            (t) => mathUtils.pointOnEllipse(
                new Vector2(x + width - rx, y + ry), rx, ry, t,
            ),
            options.subdivideThreshold,
            0.75,
            1,
        ...
        const bottomRightCorner = mathUtils.drawCurve(
            (t) => mathUtils.pointOnEllipse(
                new Vector2(x + width - rx, y + height - ry), rx, ry, t,
            ),
            options.subdivideThreshold,
            0,
            0.25,
        ...
        const bottomLeftCorner = mathUtils.drawCurve(
            (t) => mathUtils.pointOnEllipse(
                new Vector2(x + rx, y + height - ry), rx, ry, t,
            ),
            options.subdivideThreshold,
            0.25,
            0.5,
    ...
    const geometry: GeoJSON.Polygon = {
        type: "Polygon",
        coordinates: [
            ring.map((p) => svgPointToCoordinate(
