...
/**
 * Compute compass direction given a vector.
 */
export function computeBearing(v: Vector2): number {
  const angle = Math.abs(toDegrees(Vector2.angleBetween(new Vector2(0, 1), v)));
  const dotWithEast = Vector2.dot(new Vector2(1, 0), v);
  const bearing = dotWithEast >= 0 ? angle : 360 - angle;
  return bearing;
}
...
export function drawCurve(
  ...
  const angleToMidPoint = toDegrees(
    Math.abs(Vector2.angleBetween(toMidPoint, toEndPoint)),
  );
