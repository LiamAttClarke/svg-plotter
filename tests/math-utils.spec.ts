...
describe("mathUtils", () => {
    ...
    describe("lerp", () => {
        it("should return a number t percent between a and b", () => {
            expect(lerp(1, 3, 0.5)).to.equal(2);
            expect(lerp(0, 10, 1)).to.equal(10);
            expect(lerp(-3, 3, 0.5)).to.equal(0);
        });
    });
    ...
    describe("toRadians", () => {
        it("should return an angle in radians given an angle in degrees", () => {
            ...
            expect(toRadians(-360)).to.equal(Math.PI * -2);
        });
    });

    describe("toDegrees", () => {
        it("should return an angle in degrees given an angle in radians", () => {
            expect(toDegrees(Math.PI)).to.equal(180);
            expect(toDegrees(Math.PI * -2)).to.equal(-360);
        });
    });
    ...
    describe("haversineDistance", () => {
        ...
        it("should return the great-cirlce distance (in metres) between two coordinates", () => {
            ...
            expect(haversineDistance(coordA, coordB)).to.equal(9155865.3400348);
        });
    });
    ...
    describe("pointOnLine", () => {
        it("should return a point along a line", () => {
            ...
            expect(pointOnLine(pointA, pointB, 0)).to.roughly.deep.equal(new Vector2(1, 3));
            expect(pointOnLine(pointA, pointB, 0.5)).to.roughly.deep.equal(new Vector2(2, 2));
            expect(pointOnLine(pointA, pointB, 1)).to.roughly.deep.equal(new Vector2(3, 1));
        });

        it("should return the start or end point when t is out of range [0,1]", () => {
            const pointA = new Vector2(1, 3);
            const pointB = new Vector2(3, 1);
            expect(pointOnLine(pointA, pointB, -1)).to.roughly.deep.equal(new Vector2(1, 3));
            expect(pointOnLine(pointA, pointB, 2)).to.roughly.deep.equal(new Vector2(3, 1));
        });
    });
    ...
    describe("pointOnCubicBezierCurve", () => {
        it("should return a point along a cubic bezier curve", () => {
            ...
            const result1 = pointOnCubicBezierCurve(p0, p1, p2, p3, 0);
            expect(result1).to.roughly.deep.equal(new Vector2(0, 0));

            const result2 = pointOnCubicBezierCurve(p0, p1, p2, p3, 0.5);
            expect(result2).to.roughly.deep.equal(new Vector2(0.5, 0.5));

            const result3 = pointOnCubicBezierCurve(p0, p1, p2, p3, 1);
    ...
    describe("pointOnQuadraticBezierCurve", () => {
        it("should return a point along a quadratic bezier curve", () => {
            ...
            const result1 = pointOnQuadraticBezierCurve(p0, p1, p2, 0);
            expect(result1).to.roughly.deep.equal(new Vector2(0, 0));

            const result2 = pointOnQuadraticBezierCurve(p0, p1, p2, 0.5);
            expect(result2).to.roughly.deep.equal(new Vector2(0.5, 0.5));

            const result3 = pointOnQuadraticBezierCurve(p0, p1, p2, 1);
            expect(result3).to.roughly.deep.equal(new Vector2(1, 1));
        });
    });
    ...
    describe("pointOnEllipticalArc", () => {
        const arc1 = (t: number) => {
            return pointOnEllipticalArc(new Vector2(40, 50), new Vector2(120, 80), 3, 1, 0, false, false, t);
        };
        const arc2 = (t: number) => {
            return pointOnEllipticalArc(new Vector2(250, 100), new Vector2(250, 200), 120, 80, 45, true, true, t);
        };
        const arc3 = (t: number, largeArcFlag = false, sweepFlag = false) => {
            return pointOnEllipticalArc(new Vector2(250, 100), new Vector2(250, 200), 120, 80, 0, largeArcFlag, sweepFlag, t);
        };
        ...
        it("should return a point along a straight line when rx or ry is 0", () => {
            const result1 = pointOnEllipticalArc(new Vector2(0, 0), new Vector2(10, 20), 0, 80, 45, true, true, 0.5);
            const result2 = pointOnEllipticalArc(new Vector2(0, 0), new Vector2(10, 20), 120, 0, 45, true, true, 0.5);
    ...
    describe("drawCurve", () => {
        it("should return a list of points where the first element is the start position of the curve", () => {
            ...
            const curve = drawCurve((t: number) => pointOnEllipse(center, r, r, t), 1, 0, 1);
        ...
        it("should return at least the start and end points of the curve", () => {
            ...
            const curve = drawCurve((t: number) => pointOnQuadraticBezierCurve(p0, p1, p2, t), 90, 0, 1);
        ...
        it("should return an ordered list of points along a curve", () => {
            const curve = drawCurve((t: number) => pointOnEllipse(new Vector2(0, 0), 1, 1, t), 33);
            const expectedCurve = [
                new Vector2(1, 0),
                new Vector2(0, 1),
                new Vector2(-1, 0),
